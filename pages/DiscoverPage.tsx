...
const DiscoverPage: React.FC<DiscoverPageProps> = ({ hotels, reviews, friends, currentUser, allUsers, onDiscoverAction, setToast }) => {
  const [feed, setFeed] = useState<DiscoverItem[]>(() => generateDiscoverFeed(hotels, reviews, friends, currentUser, allUsers));
  ...
  useEffect(() => {
    ...
    if (feed.length <= 3) {
      setFeed(prev => {
        ...
        const newItems = generateDiscoverFeed(hotels, reviews, friends, currentUser, allUsers)
          .filter(item => !existingIds.has(item.hotel.id));

        return [...newItems, ...prev];
      });
    }
  }, [feed.length, hotels, reviews, friends, currentUser, allUsers]);
